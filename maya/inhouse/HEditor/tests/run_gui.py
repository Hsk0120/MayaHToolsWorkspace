...
if __name__ == '__main__':
    ...
    parser.add_argument('version', nargs='?', default='2027', choices=['2024', '2027'])
    parser.add_argument('--timeout', type=int, default=120)
    parser.add_argument('--shutdown-timeout', type=int, default=30)
    args = parser.parse_args()
    if args.timeout <= 0 or args.shutdown_timeout <= 0:
        parser.error('timeouts must be positive')
    directory = ROOT / '.maya-output/heditor-gui' / datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    modules = directory.with_name(directory.name + '-modules')
    modules.mkdir(parents=True)
    definition = (ROOT / 'maya/modules/HEditor.mod').read_text(encoding='ascii')
    definition = definition.replace('../inhouse/HEditor', PROJECT.as_posix())
    (modules / 'HEditor.mod').write_text(definition, encoding='utf-8')
    ...
    result = run_version(args.version, directory,
                         ...
                         suite_path=PROJECT / 'tests/gui_smoke.py',
