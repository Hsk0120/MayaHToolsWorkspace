"""ビルド済み2024/2027を隔離環境で検証する。生成物は.maya-outputへ保存。"""
import datetime
import json
import os
...
def main():
    results = []
    output = ROOT / '.maya-output/heditor-tests' / datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    for version in (sys.argv[1:] or ['2024', '2027']):
        ...
        if maya == 0:
            ...
            smoke = ROOT / '.maya-output/plugin-build/HEditor' / version / 'Release/heditor_ui_smoke.exe'
