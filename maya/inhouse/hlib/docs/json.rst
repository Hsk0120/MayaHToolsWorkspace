...
.. list-table::
   ...
   * - editor
     ...
     - 公開表示設定、タイムライン時刻・再生範囲・アニメーション範囲の保存・比較のみ
...
エディター設定は保存・読み込み・比較のみ対応します。
標準コマンドだけでは時刻や表示設定を含む全体のUndoを保証できないため、
``EditorSnapshot.apply()`` は変更前に ``NotImplementedError`` を送出します。
``plan()`` は比較結果と未対応の理由を返し、``validate().valid`` はFalseになります。
Undoを補うための独自Mayaプラグインは同梱・ロードしません。
従来形式のエディターJSONも読み込めますが、適用は同様に未対応です。
